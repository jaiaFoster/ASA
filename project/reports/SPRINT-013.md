...
## P0 follow-up: quota clock separation (verdict COMPLETE_QUOTA_CLOCK_SEPARATION)

Confirmed root cause (independently re-derived, not assumed): `ProviderRollingWindowTracker.try_reserve()` computed `window_start = now - window_seconds` from the same `_FrozenCycleClock` every pair's evaluation identity depended on. Since that clock never advances mid-cycle, `window_start` never advanced either — once a provider's declared `window_limit` reservations landed, none could ever prune, turning a real rolling rate limit into a hard per-cycle cap.

Fix: new `_MonotonicUtcClock` (anchors `time.monotonic()` to a UTC reading once, then projects every `now()` forward by real elapsed monotonic time) given to the rolling-window tracker only; `_FrozenCycleClock` untouched for evaluation timestamps and request-window identity. Merged as [#284](https://github.com/jaiaFoster/ASA/pull/284) (`6690ea2`), 10 new tests, 2830 passed full suite, zero regressions.

**Production verification (2026-08-05T17:02:47Z, first real cycle against the fix):** 82/82 pairs, 0 failed, 0 diagnostics incomplete, zero `strategy_exception`, `outcome_counts: {missing_data: 68, no_signal: 14}`. `forward_factor`'s own 30 pairs made 180+ Tradier requests before `skew_momentum`/`earnings_calendar` (52 more pairs) even began; those later pairs still show real, nonzero request counts throughout, with no exhaustion-shaped failure anywhere in the result set — the signature a still-broken frozen-clock cap would produce (a late block of pairs going to zero) is absent. Consistent with quota genuinely regaining capacity as real time elapsed during the cycle, not merely surviving by luck. No payloads or credentials referenced — evidence is cycle-summary counts only.

**Full trading day closed out (2026-08-05):** the two remaining scheduled slots also ran clean against the fix — 19:04:09Z (`total: 82, failed: 0, outcome_counts: {missing_data: 75, no_signal: 7}`) and 19:52:23Z, the session's own close-10m slot (`total: 82, failed: 0, outcome_counts: {missing_data: 72, no_signal: 10}`). All 3 cycles that ran against #284 today: 82/82 pairs, 0 failed, 0 diagnostics incomplete, zero `strategy_exception`, every time. The cron window closes at 20:00 UTC; no further slots fire today.
