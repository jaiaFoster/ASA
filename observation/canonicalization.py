...
ASA-CORE-002. Internal to the Observation Layer — not a broad application
API. The canonical form makes logically equivalent normalized values
structurally identical; the serialization makes them byte-identical, which
is what Observation identity (``observation/identity.py``) hashes.
...
Serialization contract (versioned via the identity algorithm, v1):
