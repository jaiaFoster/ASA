...
def test_subject_plan_only_imports_from_permitted_roots() -> None:
    ...
    # -- both pure stdlib, no new package dependency. logging added
    # (production-root wiring checkpoint): SubjectAcquisitionPlan's own
    # attempt-recording resilience (attempt_recording_degraded) logs a
    # best-effort warning on a persistence-repository failure instead of
    # raising -- also pure stdlib, no new package dependency.
    permitted = {"__future__", "dataclasses", "logging", "market_data", "typing"}
    tree = ast.parse(_MODULE.read_text())
    roots = _imported_roots(tree)
